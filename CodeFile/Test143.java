...
● 캐릭터 셋(Character Set)
   ...
   ○ UTF-8과 UTF-16
	  ...
	  - UTF-8(8bit UCS Transformation Format)
	    웹에서 기본적으로 사용하는 코드.
		UCS-2로 정의되는 문자 집합을 이요하여 기술된 문자열을
		바이트 열로 변환하는 방식의1개 1문자를 1~6바이트 가변장
		멀티바이트로 변환한다.

	  - UTF-16(16bit UCS Tranformation Format)
...
public class Test143
{
	public static void main(String[] args) throws UnsupportedEncodingException
	{
		...
		System.out.println("Length : " + buf.length);
		//--==>> UnsupportedEncodingException 에러 발생(컴파일 에러)
		// → throws UnsupportedEncodingException 작성
		//    import java.io.UnsupportedEncodingException; 작성
        //--==>> Length : 9
		...
		// new String("김태형");                   → 문자열 객체 만들어짐
		// new String(바이트배열, 인코딩방식);     → 문자열 객체 만들어짐
		...
		System.out.println("Value 2 : " + new String(buf)); // 세 개씩 묶은 걸 두 개씩 묶자
		//--==>> Value 2 : ?븳異⑺씗

		System.out.println(); // 개행 -------------------------------------------------------------

		String convert = new String(name.getBytes("euc-kr"), "utf-8"); // 변환한답시고 이렇게 하면 안 된다.
