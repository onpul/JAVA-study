...
○ Vector(벡터)
   ...
   배열과 비슷하지만, 배열과 달리 다른 종류의 데이터 형을 가진               // 배열의 중요한 성질과는 차이가 있음
   데이터 요소를 가질 수 있으며, 데이터가 가득차면 자동으로 저장 영역을      // 배열 그림과 같다 생각하면 됨
   확장시키게 된다.                                                          // --> 다른 종류 가능, 길이 가변

   또한, 객체 레퍼런스를 저장하기 때문에 // 벡터는 프리미티브 자료형을 요소로 취할 수 없음. 오로지 개체만.
   JDK 1.5 이전에는 자바 기본형 데이터는 저장할 수 없었으나
   JDK 1.5 이후부터는 오토박싱 / 오토언박싱 기능이 지원되면서
   기본 데이터형도 저장이 가능하다.

   벡터 클래스의 기본 구조는 
   시퀀스(Sequence) 데이터 구조에 기반하며 // 시퀀스 --> 일련의 사건, 일련의 순서 / ex) 은행 번호표, 맛집 대기표
   시퀀스 데이터 접근을 순차적인 인덱스 값에 대한 위치로 접근하게 된다.
   따라서, 자료구조의 양 끝단에서만 접근이 가능한 큐나
   한쪽 끝에서만 접근이 가능한 스택과 달리
   시퀀스 개념의 인덱스 값을 이용하여 자료 구조의 임의의 지점에서 // 배열 상상하기
   저장 및 접근이 가능하다.
   ...
   - void removeElementeAt(int index)
   - E remove(int index)
     주어진 위치의 객체를 제거한다.
   ...
   - void setSize(int newSize)
     벡터의 크기를 조절한다.
	 만약 주어진 크키가 현재 크기보다 작다면 나머지는 버린다.
