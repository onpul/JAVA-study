...
//    상위 클래스의 생성자를 정의하지(작성하지)않거나 == 상위 클래스에 default 생성자 만들어짐
//    인수가 없는 생성자만을 정의한(작성한) 경우 == 상위 클래스에 default 생성자 만들어짐
//    명시적으로 하위 클래스에서 상위 클래스의 생성자를 호출하지 않아도 == super() 넣지 않아도
//    아무런 문제가 발생하지 않지만
...
{
	// 1. 부모 클래스의 내용을 그대로 물려 받음
	// 2. 생성자는 제외되니까 빠진다
	// 3. private 멤버는 접근 불가니까 상속도 안 됨
	...
	*/
	...
	public Rect114()
	{	
		// 자동으로 (부모 생성자를 호출하는 구문) 삽입
		// SuperTest114()가 아니라 super()!!!
		//super(); → SuperTest114();
	}
	...
	@Override //-- 어노테이션(annotation) - metadata - JDK 1.5
	          // 컴파일 과정까지 남아있는 주석(컴파일러에게 알리는)
	          // 없다고 에러 나진 않음, 자바가 일하기 쉽도록
	public void write()
	{	
